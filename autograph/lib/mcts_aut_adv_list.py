...
class MCTSAutAdvList(Generic[T]):
    ...
    def pick_action(self, state: T, root_state: bool) -> int:
        ...
        #even team is hiding, odd team is seeking
        #if self.hideAndSeek:
        #    if (cur_turn % 2 == 1 and game_step < 200):
        #        return 4 #whatever action is stand still action
        # this is \pi_tree eqn implementation (7)
        cur_turn = state[3]
    ...
    def find_leaf(self, envs: List[AutShapingWrapperAdv], cur_state: T, aut_states: List[Set[int]],
        ...
        while not done and not self.is_leaf(cur_state):
            ...
            for i in range(0, len(self.aut_stats)):
                next_state, rewards[i], dones[i], info[i] = envs[i].step(action)
                ...
                if i == turn:
                    next_state_of_turn = next_state

                # SHOW TREE STEP
                if show_tree_step and i == 0:
                    print("tree step " + str(treestep))
                    treestep += 1
                    envs[0].render()
            ...
            fullState = envs[0].get_full_state()

            #            single state, action,
            #                           list of rewards
            #                                         single next state
            #                                                     list of edges
            #must cast to tuple() so its a hashable type


            turns.append((cur_state, action, tuple(rewards), next_state_of_turn, tuple(aut_edges)))

            if self.c_sigma != 0:
                if done or next_state_of_turn in seenstates:
                    done = True
                    self.sigma[cur_state][action] = 0
                else:
                    seenstates.add(next_state_of_turn)

            cur_state = next_state_of_turn
    ...
    def find_leaf_batch(self, envs: List[AutShapingWrapperAdv], num_leaves: int, start_state: T) \
...
#TODO curiousity for multiagent
    ...
    def mcts_mini_batch(self, envs: List[AutShapingWrapperAdv], start_state: T, batch_size: int) -> None:
        """
    ...
    def backup_mcts_trace(self, sars: BackupTrace, final_value_estimates: List[float]) -> List[float]:
        ...
        for state, action, rewards, next_state, aut_states in reversed(sars):
            ...
            # calculate for each agent. adjusted values in set indexed by state->action->agent
            for i in range(0, len(self.aut_stats)):
                self.state_action_aut_total[state][action][i] += self.aut_stats[i].v(aut_states[i])
                self.state_action_aut_average[state][action][i] = \
                    self.state_action_aut_total[state][action][i] / self.visit_count[state][action]
            ...
            for idx, ((storage, treeop), prev_values) in enumerate(zip(self.tree_backups, prev_tree_values)):
                if prev_values is not None:
                    if type(prev_values) is not float and len(prev_values) > 1:
                        if prev_values[0] is not None:
                            for agent in range(0, len(self.aut_stats)):
                                storage[state][action][agent] = \
                                        treeop.edge_combinator(storage[state][action][agent], prev_values[agent])
                    else:
                        if prev_values is not None:
                            for agent in range(0, len(self.aut_stats)):
                                storage[state][action] = \
                                        treeop.edge_combinator(storage[state][action], float(prev_values))

                        #get node_value from both agents?

                if idx == 3:
                    storage_split = list(list(storage[state][action][player] for action in range(0, self.num_actions))
                                         for player in range(0, len(self.aut_stats)))

                    for agent in range(0, len(self.aut_stats)):
                        prev_tree_values[idx][agent] = treeop.node_value(storage_split[agent])
                else:
    ...
    def mcts_batch(self, envs: List[AutShapingWrapperAdv], state: Any, count: int, batch_size: int) -> None:
        ...
        for _ in range(count):
            self.mcts_mini_batch(envs, state, batch_size)
    ...
    def get_policy_value(self, state: T, tau: float = 1) -> Tuple[List[float], List[List[float]]]:
        ...
        values = list(list(self.value_avg[state][action][turn] for action in range(0, self.num_actions)) for turn in range(0, len(state[0])))
        #get policy for player that is playing
        return probs, values

    # returns pi_play
    def get_policy_valueOld(self, state: T, tau: float = 1) -> Tuple[List[float], List[float]]:
        """
        Extract policy and action-values by the state
        :param state: state of the board
        :param tau: temperature (as defined in alphago zero paper)
        ...
        # agent 1 turn
            ...
            counts = self.visit_count[state]
            if tau == 0:  # Set the worst action (for agent0) to 1, others at 0
                probs = [0.0] * len(counts)
                probs[np.argmin(counts).item()] = 1.0
            else:  # Make the best action stand out more or less
                counts = [count ** (1.0 / tau) for count in counts]
                total = sum(counts)
                probs = [count / total for count in counts]
