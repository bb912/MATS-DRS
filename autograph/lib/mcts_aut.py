...
class AutStats(AbstractAutStats):
    ...
    def v(self, state):
        ...
        # Calculate V_LTL - what is the max term?
...
class MCTSAut(Generic[T]):
    ...
    def pick_action(self, state: T, root_state: bool) -> int:
        ...
        aut_avg = [self.aut_stats.baseline() if a is None else a for a in aut_avg] # if no V_LTL, use baseline, i.e. the min in (6) for N=0
        ...
        # Calculation of score in same order
        # this is (7) (with some more stuff)
        score = [
    ...
    def find_leaf(self, env: Env, cur_state: T, aut_state: Set[int]) -> Tuple[BackupTrace, T, bool]:
        ...
        while not done and not self.is_leaf(cur_state):
            ...
            # done means that an acceptance state is reached (or we can never reach one)
        ...
        for i in range(num_leaves):
            ...
            if done:
                ...
                if end_state not in will_create:
                    ...
                    # Add the trajectory along with the leaf
    ...
    def mcts_mini_batch(self, env: AutShapingWrapper, start_state: T, batch_size: int) -> None:
        ...
        if len(creates) > 0:
            ...
            # When a state is created, we get pi_CNN and V_CNN
        ...
        for state, bup, (policy, value) in zip(create_states, create_trajectories, state_infos):
            self.create_state(state, policy) # policy is pi_CNN
            backups_with_final_values.append((bup, float(value))) # this contains the trajectories with their final values, i.e. V_CNN
    ...
    def backup_mcts_trace(self, sars: BackupTrace, final_value_estimate: float = 0.0) -> float:
        ...
        """
        value_discounted = final_value_estimate # final_value_estimate is V_CNN
        ...
        for state, action, reward, next_state, aut_state in reversed(sars):
            ...
            #
            #  state_action_aut_average is used in pick_action
            ...
            value_discounted += reward # reward is 1 if we found an accepting node in the automatan
                                       # reward is only 1 at the last node of the trajectory, and 0 for all other nodes

            self.value[state][action] += value_discounted # Discounted V_CNN
