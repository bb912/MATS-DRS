...
class MineWorldConfig:
    ...
    def __init__(self, shape: Tuple[int, int], num_teams: int, num_agents: int,
        ...
        self.stationary_teams = stationary_teams
    ...
    def from_dict(dict, team_num: int):
        ...
        stationary_teams = dict.get("stationary_teams", [False] * num_teams)
        ...
        return MineWorldConfig(shape=shape, num_teams=num_teams, num_agents=num_agents,
                               ...
                               init_directions=init_directions, init_teams=init_teams, stationary_teams=stationary_teams,
                               )
...
class MineWorldEnv(GridEnv, SaveLoadEnv):
    ...
    def get_team_turn(self):
        return self.turnn % self.num_teams
    ...
    def step(self, action: int):
        ...
        if stepper.dead == 1 or self.config.stationary_teams[stepper.team] is True:
            action = 4
            # temporary way to kill an agent?

        if action < 5:

            #for cone of vision direction
            if action < 4:
                stepper.direction = action

            # Movement or no-op, action 4 represents no operation as agent will attempt to move out of bounds
            action_offsets = [(0, -1), (1, 0), (0, 1), (-1, 0), (1000, 1000)]
            new_place = element_add(stepper.position, action_offsets[action])
            ...
            if can_move and stepper.team == 0:
                ...
                if attack:
                    ...
                    try:
                        #picks up whatever is in the position the agent is moving to
                        if (this_tile.ap_name == 'System' and pick_up_system) or this_tile.ap_name != 'System':
                            ...
                            if this_tile.ap_name == 'Keycard' and stepper.team == 1:
                                ...
                                if over_capacity and this_tile.wall:
                                    can_move = False
        ...
        if action == 5:
            ...
            if stepper.team == 1:  
                #if two guards stand here perform demolition,
                #demolition destroys all boxes adjacent to this sell
                count_seekers = 0
                count_tools = 0
                for x in range(len(self.agents)):
                        agent = self.agents[x]
                        if agent.position == stepper.position:
                            if agent.team == 1:
                                count_seekers += 1
                if count_seekers >=2:
                    #Condition for demolition met
                    for offset in action_offsets:
                        #loop through adjacent cells
                        pos_check = element_add(stepper.position, offset)
                        if pos_check in self.special_tiles:
                            this_tile: MineWorldTileType = self.special_tiles[pos_check]
                            inBounds = self._in_bounds(pos_check)
                            if this_tile.ap_name == 'Box' and inBounds:
                                # break the wall
                                del self.special_tiles[pos_check]

            if stepper.position in self.special_tiles and stepper.team == 0:
                this_tile: MineWorldTileType = self.special_tiles[stepper.position]
                #pick whatever is here if conditions are met
                pick_up_system = False
                if this_tile.ap_name == 'System' and stepper.team == 0:
                    count_keycards = 0
                    for x in range(len(self.agents)):
                    ...
                    if count_keycards > 0:
                        pick_up_system = True
                try:
                    #picks up whatever is right under the agent
                    if (this_tile.ap_name == 'System' and pick_up_system) or this_tile.ap_name != 'System':
                        if this_tile.ap_name == 'Keycard' and stepper.team == 1:
                            pass
                        else:
                            ...
                            self.inventoryCounters[stepper.playerNum] = stepper.inventory.copy()
                            if this_tile.consumable and not over_capacity:
                                del self.special_tiles[stepper.position]
                        
                except ValueError:  # Couldn't apply inventory
                    pass

            elif stepper.team == 0:
                # Not on a special tile, infiltrators can attempt to place/build box here if no other agents are standing here
                canBuild = True
                #check if there is an agent on position we want to build on
                for x in range(len(self.agents)):
                    agent = self.agents[x]
                    if agent.playerNum == x:
                        continue
                    if agent.position == stepper.position:
                        canBuild = False
                if canBuild:
                    ...
                    for inv_config in self.inventories[stepper.playerNum]:
                        if inv_config.name == 'box':
                            ...
                            if stepper.inventory[inv_config.name] > 0:
                                ...
                                self.special_tiles[stepper.position] = \
