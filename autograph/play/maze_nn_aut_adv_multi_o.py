...
#IGNORE ME, I"M LEFTOVER GARBAGE
def run_aut_episode(nets: List[torch.nn.Module], envs: List[AutShapingWrapperAdv], max_length: int, max_len_reward: Optional[int],
...
def run():
    ...
    try:
        ...
        cmans = list(CheckpointManager(CHECKPOINT_PATHS[i] + "_copy", LOAD_FROM_CHECKPOINT, SAVE_CHECKPOINTS, device=DEVICE)
                     for i in range(0, NUM_PLAYERS))
    ...
    envs = list(AutShapingWrapperAdv(orig_env, AUT_PARAM_FUNCS[i], auts[i],
    ...
    nets = list(cman.load("net", train_net_creator(orig_env, **NETWORK_PARAMS),
    ...
    if CURIOSITY_NET:
        ...
        icm_list = list(cman.load("icm", curiosity_net_creator(orig_env, **CURIOSITY_PARAMS), StateDictLoadHandler()).to(
    ...
    for idx, cman in enumerate(cmans):
        ...
        if TRANSPLANTS[idx]:
            orig_alt_stats = cman.load_from_alt("aut_stats", AutStats(len(auts[idx].graph.network)), StateDictLoadHandler())
            wrapped_aut_stats[idx] = aut_transplant_anneals[ANNEAL_AUT_TRANSPLANTS[idx]](orig_alt_stats, aut_stats_list[idx],
                                                                              **ANNEAL_AUT_TRANSPLANTS_PARAMS[idx])
