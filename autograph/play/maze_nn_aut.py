...
def run_mcts_aut_episode(net: torch.nn.Module, env: AutShapingWrapper, max_length: int,
    ...
    mcts = MCTSAut(env.action_space.n, curiosity_evaluator, state_evaluator, curiosity_trainer, c_puct=c_puct,
...
def run_aut_episode(net: torch.nn.Module, env: AutShapingWrapper, max_length: int, max_len_reward: Optional[int],
...
def run():
    ...
    env = AutShapingWrapper(orig_env, AUT_PARAM_FUNCS, aut, use_potential=False, **AUT_OTHER_PARAMS)
